...
public class AIDialog {
    ...
    public AIDialog(final Context context, final AIConfiguration config, final int customLayout) {
        ...
        dialog.setOnDismissListener(new DialogInterface.OnDismissListener() {
            ...
            public void onDismiss(DialogInterface dialogInterface) {
                //start wake word recognizer when listner dialog is closed as we are not longer listing using google recognizer
                startWakeWordRecognizer();
                titleTextView.setText(context.getResources().getString(R.string.initializing_feedback));//clear title text
    ...
    final Runnable closeDialog = new Runnable() {
        ...
        @Override
        public void run() {
            AIDialog.this.close();; // hide dialog
        }
    };
    ...
    private void removeDialogCloseCallback(){
        if(handler != null){
        ...
        aiButton.setResultsListener(new AIButton.AIButtonListener() {
            ...
            public void onError(final AIError error) {
                ...
                handler.post(new Runnable() {
                    ...
                    public void run() {
                        try {
                            titleTextView.setText(context.getString(R.string.error_feedback, error.getMessage()));
                        }
                        catch (Exception ex){
                ...
                //close dialog after 5 seconds of no interaction
                handler.postDelayed(closeDialog, 5000); // start a new hiding process that will trigger after 5 seconds
            ...
            public void onFinished() {
                ...
                if (resultsListener != null) {
                    resultsListener.onFinished();
                }
                handler.post(new Runnable() {
                    @Override
                    public void run() {
                        titleTextView.setText(R.string.thinking_feedback);
                    }
                });
                handler.postDelayed(closeDialog, 10000); // start a new hiding process that will trigger after 10 seconds
            ...
            public void onStarted() {
                removeDialogCloseCallback();
                if (resultsListener != null) {
                    resultsListener.onStarted();
                }
                handler.post(new Runnable() {
                    @Override
                    public void run() {
                        titleTextView.setText(R.string.listening_feedback);
                    }
                });
            }
    ...
    private void startWakeWordRecognizer() {
